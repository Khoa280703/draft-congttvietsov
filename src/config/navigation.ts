import { INTERNAL_MENU_ITEMS, MAIN_NAVIGATION_ITEMS } from "./menu";
import { PUBLIC_ROUTES, INTERNAL_ROUTES, ROUTES } from "./routes";

// Create URL to navigation item mapping for public
export const PUBLIC_URL_TO_NAV_ITEM = MAIN_NAVIGATION_ITEMS.reduce(
  (acc, item) => {
...
export const PUBLIC_NAV_ITEM_TO_URL = MAIN_NAVIGATION_ITEMS.reduce(
  (acc, item) => {
