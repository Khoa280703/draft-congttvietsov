...
export const useStickyScroll = (
  ...
  // Wrapper minHeight: đủ để scroll effect hoạt động, nhưng không quá lớn
  // Giới hạn tối đa để tránh khoảng trắng quá lớn
  const wrapperMinHeight = Math.min(
    Math.max(defaultHeight, sectionHeight * 1.2),
    sectionHeight + 400 // Giới hạn tối đa = sectionHeight + 400px
  );
