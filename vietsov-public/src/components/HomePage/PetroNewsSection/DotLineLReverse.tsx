...
interface DotLineLReverseProps {
  dotColor?: string;
  lineColor?: string;
  dotSize?: number;
  lineWidth?: number;
...
const DotLineLReverse: React.FC<DotLineLReverseProps> = ({
  dotColor = "#F07B22",
  lineColor = "#222E50",
  dotSize = 12,
  lineWidth = 2,
  ...
  // Calculate right position for vertical line based on horizontal line width
  // When horizontal line is at left-0 with width W, its right edge is at right: calc(100% - W)
  const verticalLineRight = isHorizontalMotionValue
    ? useTransform(
        horizontalLineWidth as MotionValue<string>,
        (width) => {
          // Convert percentage or pixel value to right position
  ...
  return (
    <div className={`relative ${className}`} style={{ minHeight: "120px" }}>
      {/* Horizontal Line - Top, extends from left */}
      ...
      {/* Vertical Line - From top right of horizontal line down */}
      ...
      {isVerticalMotionValue ? (
        <motion.div
          ...
          style={{
            ...
            right: verticalLineRight || "0px",
            transform: `translate(${dotSize / 2 - lineWidth / 2}px, -${dotSize / 2}px)`,
            zIndex: 2,
          }}
        />
      ) : (
...
export default DotLineLReverse;
